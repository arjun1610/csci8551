...
%
% This is the implementation of image edge detection using ant colony, based on
...
% image loading
filename = 'eye';
img=rgb2gray(imread([filename '.bmp']));
img = double(img)./255;
[nrow, ncol] = size(img);
...
for nMethod = 3:3;
    ...
    ant_total_num = 2*round(sqrt(nrow*ncol));
...
%     ant_pos_idx(:,2) = round(1 + (ncol-1) * temp(:,2)); %column index
    ...
    %function
    [~,index]=max(v);
    ant_pos_idx(1:ncol,1)= 1:ncol;
    temp=0:ncol-1;
    temp1=ones(1,ncol)*ncol;
    ant_pos_idx(ncol+1:end,1)=temp1-temp;
   
    ant_pos_idx(1:nrow,2)=index;
    ant_pos_idx(nrow+1:end,2)=index;
    ...
    imwrite(uint8(abs((p>=T).*255)), gray(256), [filename '_edge_aco_' num2str(nMethod) '_' num2str(iteration_idx) '.bmp'], 'bmp');        
